...
@Component({
  selector: 'app-document-edit-abonne',
  templateUrl: './document-edit-abonne.component.html'
})
export class DocumentEditAbonneComponent implements OnInit {
    ...
    public prepareEdit() {
        this.item.uploadDate = this.service.format(this.item.uploadDate);
        this.item.dateLastUpdate = this.service.format(this.item.dateLastUpdate);
    }
 ...
 public edit(): void {
   ...
   public addDocumentFields() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.documentFieldsElement.id == null){
                this.item.documentFields.push(this.documentFieldsElement);
            }else{
                const index = this.item.documentFields.findIndex(e => e.id == this.documentFieldsElement.id);
                this.item.documentFields[index] = this.documentFieldsElement;
            }
   ...
   public async openCreateField(field: string) {
        const isPermistted = await this.roleService.isPermitted('Field', 'edit');
        if (isPermistted) {
   ...
   public async openCreateDocumentFieldState(documentFieldState: string) {
        const isPermistted = await this.roleService.isPermitted('DocumentFieldState', 'edit');
        if (isPermistted) {
   ...
   public async openCreateDocumentCategorie(documentCategorie: string) {
        const isPermistted = await this.roleService.isPermitted('DocumentCategorie', 'edit');
        if (isPermistted) {
   ...
   public async openCreateDocumentState(documentState: string) {
        const isPermistted = await this.roleService.isPermitted('DocumentState', 'edit');
        if (isPermistted) {
